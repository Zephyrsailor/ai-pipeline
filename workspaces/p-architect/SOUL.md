...
```
把 repo URL 更新到 state.json：
```bash
cd /Users/zephyr/Desktop/lab/deep-research/ai-pipeline
python3 -c "
import json
with open('projects/{slug}/.pipeline/state.json') as f: d=json.load(f)
d['repo_url'] = 'https://github.com/Zephyrsailor/{slug}'
with open('projects/{slug}/.pipeline/state.json','w') as f: json.dump(d,f,indent=2)
"
```
